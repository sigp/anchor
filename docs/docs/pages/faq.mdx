...
Anchor needs no flag: it always creates validator registrations. To actually use external builders,
configure the builder or MEV-Boost connection on your beacon node. The `--builder-proposals` flag
that used to gate this is deprecated and ignored, and will be removed.

See [MEV Configuration](/mev_configuration) for tuning, including `--proposer-delay-ms`.
