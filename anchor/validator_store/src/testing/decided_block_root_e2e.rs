...
use types::{
    BeaconBlock, BeaconBlockElectra, BeaconBlockGloas, EmptyBlock, ForkName, Hash256,
    MainnetEthSpec, Slot,
};
...
/// Builds a Gloas harness and returns it with the pubkey of its single validator.
...
fn gloas_block(harness: &ValidatorStoreTestHarness) -> BeaconBlock<MainnetEthSpec> {
    let mut block = BeaconBlockGloas::<MainnetEthSpec>::empty(&harness.spec);
...
async fn pre_gloas_duty_creates_no_entry() {
    ...
    let pubkey = harness.validator_pubkey(0, 0);
    let mut electra_block = BeaconBlockElectra::<MainnetEthSpec>::empty(&harness.spec);
