...
mod tests {
    ...
    async fn test_validator_added_event_processing() {
        ...
        // Create properly formatted shares data: [signature | public keys | encrypted keys]
        // Use the valid signature data from the existing test_keysplitter test
        // This ensures we have a valid BLS signature that will pass verification
        ...
        let shares_data = hex::decode(share_data).expect("Failed to decode hex string");
