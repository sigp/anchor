...
/// Split file content into the portion before the start marker and the portion
/// after the end marker (inclusive of trailing content).
...
mod tests {
    ...
    #[test]
    fn test_split_at_markers_correctly_splits_content_at_markers() {
        let content = format!(
            "Intro text\n{CLI_REFERENCE_START}\nGenerated content\n{CLI_REFERENCE_END}\nOutro text"
        );
