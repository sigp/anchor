...
pub const GET_ALL_CLUSTERS: &str = "SELECT DISTINCT
            c.cluster_id,
            c.owner,
            o.fee_recipient,
            c.liquidated
...
// Validator
pub const INSERT_VALIDATOR: &str = "INSERT INTO validators (validator_pubkey, cluster_id, validator_index, graffiti) VALUES (?1, ?2, ?3, ?4)";
...
// Shares
pub const INSERT_SHARE: &str = "INSERT INTO shares
            (validator_pubkey, cluster_id, operator_id, share_pubkey, encrypted_key)
         VALUES
            (?1, ?2, ?3, ?4, ?5)";
pub const GET_SHARES: &str = "SELECT share_pubkey, encrypted_key, operator_id, cluster_id, validator_pubkey FROM shares WHERE operator_id = ?1";
...
pub const BUMP_NONCE: &str = "INSERT INTO nonce (owner, nonce) VALUES (?1, 0)
