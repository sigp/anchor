...
mod tests {
    ...
    fn test_duty_calculation_mathematical_properties() {
        ...
        for i in 2..=20 {
            ...
            assert!(
                ...
                "Duties should be non-decreasing for small committees: {} -> {}",
                prev_duties,
                current_duties
            );
    ...
    fn test_message_rate_for_different_committee_configurations() {
        ...
        for (committee_size, num_validators) in configs {
            ...
            assert!(
                rate > 0.0,
                "Rate should be positive for committee_size={}, num_validators={}",
                committee_size,
                num_validators
            );
            assert!(
                rate.is_finite(),
                "Rate should be finite for committee_size={}, num_validators={}",
                committee_size,
                num_validators
            );
            assert!(
                rate < 10000.0,
                "Rate should be reasonable for committee_size={}, num_validators={}",
                committee_size,
                num_validators
            );
        }
